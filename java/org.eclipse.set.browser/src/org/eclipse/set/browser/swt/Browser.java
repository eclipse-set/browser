...
public class Browser extends Composite {
	...
	static final String PACKAGE_PREFIX = "org.eclipse.set.browser."; //$NON-NLS-1$
	 ...
	 * the web browser
	 */
	public org.eclipse.set.browser.WebBrowser webBrowser;
	...
	/**
	 * Attempts to dispose the receiver, but allows the dispose to be vetoed by
	 * the user in response to an <code>onbeforeunload</code> listener in the
	 ...
	 *                causes a javascript error to be thrown</li>
	 *                <li>ERROR_INVALID_RETURN_VALUE when the script returns a
	 *                value of unsupported type</li>
	 *                <li>ERROR_THREAD_INVALID_ACCESS when called from the wrong
	 *                thread</li>
	 *                <li>ERROR_WIDGET_DISPOSED when the widget has been
	 *                disposed</li>
	 ...
	 * Returns the result, if any, of executing the specified script.
	 * <p>
	 * Evaluates a script containing javascript commands. When
	 * <code>trusted</code> is <code>true</code> script is executed in the
	 * context of Chrome with Chrome security privileges. When
	 * <code>trusted</code> is <code>false</code> script is executed in the
	 * context of the current document with normal privileges.
	 * </p>
	 * <p>
	 * If document-defined functions or properties are accessed by the script
	 * then this method should not be invoked until the document has finished
	 * loading (<code>ProgressListener.completed()</code> gives notification of
	 * this).
	 * </p>
	 * <p>
	 * If the script returns a value with a supported type then a java
	 ...
	 *            the script with javascript commands
	 * @param trusted
	 *            <code>true</code> or <code>false</code> depending on the
	 ...
	 * Note that this may not reflect the javascript enablement on the
	 * currently- viewed page if <code>setJavascriptEnabled()</code> has been
	 * invoked during its lifetime.
	 *
	 ...
	 *         <p>
	 *         Note, the exact return value is platform dependent. For example
	 *         on Windows, the returned string is the proccessed webpage with
	 *         javascript executed and missing html tags added. On Linux and OS
	 *         X, this returns the original HTML before the browser has
	 *         processed it.
	 *         </p>
	 *
	 * @exception SWTException
	 ...
	 *
	 * @since 3.3
	 * @deprecated SWT.MOZILLA is deprecated and XULRunner as a browser renderer
	...
	/**
	 * Renders a string containing HTML. The rendering of the content occurs
	 * asynchronously. The rendered page can be given either trusted or
	 ...
	 * granted to the rendered page. Specifying <code>true</code> for trusted
	 * gives the page permissions equivalent to a page on the local file system,
	 * while specifying <code>false</code> for trusted gives the page
	 * permissions equivalent to a page from the internet. Page content should
	 * be specified as trusted if the invoker created it or trusts its source,
	 * since this would allow (for instance) style sheets on the local file
	 * system to be referenced. Page content should be specified as untrusted if
	 * its source is not trusted or is not known.
	 *
	 * @param html
	 *            the HTML content to be rendered
	 * @param trusted
	 *            <code>false</code> if the rendered page should be granted
	 ...
	 *            the URL to be loaded
	 * @param postData
	 *            post data to be sent with the request, or <code>null</code>
	 * @param headers
	 *            header lines to be sent with the request, or <code>null</code>
	 *
	 * @return <code>true</code> if the operation was successful and
	 *         <code>false</code> otherwise.
	 *
