...
public interface OpenWindowListener extends SWTEventListener {
	 ...
	 * <p>
	 * A standalone system browser is used to host the new window if the
	 ...
	 * <code>true</code> on platforms that don't support a standalone system
	 * browser for new window requests.
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * The following fields in the <code>WindowEvent</code> apply:
	 * </p>
	 * <ul>
	 * <li>(in/out) {@link WindowEvent#required}: true if the platform requires
	 * the user to provide a <code>Browser</code> to handle the new window or
	 * false otherwise.
	 * <li>(out) {@link WindowEvent#browser}: the new (unique)
	 * <code>Browser</code> that will host the content of the new window.
	 * <li>(in) widget the <code>Browser</code> that is requesting to open a new
	 * window
	 * </ul>
	 *
	 * @param event
	 *            the <code>WindowEvent</code> that needs to be passed a new
